...
> **Known gaps (don't treat the clean state as reality):** checkout now charges **per-song USD** via `computeOrderPrice` (S1 #16, live since `20260702`; the one-shot first-mix discount rides as a private 50% code, floor-bounded to $225/song) with **T&C consent recorded** (#23, `20260704`) — but it charges **no tax** (`taxCents = 0` stub; implementation is #31). Lifecycle transitions are guarded (#34 — `canTransition` in `workflow.ts` + the `20260705` DB status fence), but **two payment-write holes remain**: a client can PostgREST-INSERT a forged-`paid_at` project (#40), and the payment-status poll's claim write has been dead since `20260702` (#41 — paid clients wait on the webhook). The discount-codes catalog + studio admin CRUD exists (#17, `20260704`) but is **client-inert** — redemption at checkout is #25/#26. IndexNow shipped code-only and is **inert in production** (`INDEXNOW_KEY` unset; activation is #33). Still **planned, not built**: add-ons (#19), order email (#24), the purge job (#27), the homepage price calculator (#30), and the stem-prep guide the T&C references (#32). Marketing copy still advertises the old 50%-off promo until S10 (#9) — the charge and the copy disagree for single-song first orders.
