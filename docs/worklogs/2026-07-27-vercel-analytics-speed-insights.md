...
Both are inert until their per-project toggles are flipped in the Vercel dashboard — free on the Hobby tier, but the dashboard sits with the client's account, the same access gate holding #33 and the #27 cron. Until then the components render nothing and send nothing. Lint and build green; no schema change. (Resolved later the same day once access was recovered: toggles on, both script endpoints serving 200 in prod.)
