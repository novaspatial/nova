...
**Done =** the 10 open work issues below closed (or consciously moved to *Deliberately not built*) and the open decisions in #1 answered.
...
**Label key:** `ready-for-agent` = fully specified, take it AFK · `ready-for-human` = an input/decision is needed first (see the gate column) · `needs-info` = waiting on Mike/Jamie · `needs-triage` + `architecture` = the review refactors still awaiting Onur's triage confirmation — now just #36 and #39 (#34 shipped 2026-07-04, #38 2026-07-13; #35/#37 flipped `ready-for-agent` 2026-07-24) · `needs-triage` + `bug` = #42/#43, the insert-fence residuals from the #40/#41 hardening (shipped 2026-07-08), same gate · `bug` = live defect.
...
| --- | --- | --- | --- |
| 1 | [#35](https://github.com/novaspatial/nova/issues/35) + [#37](https://github.com/novaspatial/nova/issues/37) **arch** | **Storage seam** (buckets/paths/signed-URL choreography, one upload-runner hook, server-side size/MIME) + **`requireProjectChild`** authz helper. One slice — the download handlers consume both. | `ready-for-agent` (triage go 2026-07-24). Do before #27 (purge sweeps buckets) and #13 (admin download consumes `signedDownload`). **Re-anchor first:** both issues' comments map the drift — #34/#40/#41/#26/#24 touched the cited handlers; replace only preambles, never the `claimProjectPayment`/consume/receipt choreography. ADR-0002/0003 preserved → no migration expected. |
| 2 | [#33](https://github.com/novaspatial/nova/issues/33) **S23** | **Activate IndexNow in prod**: set `INDEXNOW_KEY`, verify live key + accepted ping, and **reconcile the apex→www redirect with D10**. | Pure ops + verification; no code gaps. `ready-for-human` — needs prod env access (`INDEXNOW_KEY`, DNS/redirect). |
| 3 | [#36](https://github.com/novaspatial/nova/issues/36) **arch** | **Extract `useCommentClock`** — first meaningful test coverage for the Listen step. | Independent; fill capacity between gated slices. Strength: Strong. |
| 4 | [#42](https://github.com/novaspatial/nova/issues/42) + [#43](https://github.com/novaspatial/nova/issues/43) **bug** | **Insert-fence residuals** from the #40/#41 work. #42: delete-then-reattach of a freed Stripe intent id (needs a DB-level floor — intent tombstone or service-mediated Stripe insert; the #19 `add_ons` metadata check narrows the forgeable surface further but the same-shape resurrection stands). #43: direct PostgREST `pending_payment` inserts skip the route's rate limit + consent gate, and (since #26) can stamp an `applied_coupon_code` without a hold — if such a row is ever paid, consume burns that code's capacity (griefing-only; no price advantage). | Narrow, non-payment-integrity; `needs-triage`. Take with the arch lane. |
...
(#23, the former no-gate top of this path, shipped 2026-07-04; #40/#41 shipped 2026-07-08; #9/#31/#38/#25 shipped 2026-07-13; #26 shipped 2026-07-15; #30, #19, and #24 shipped 2026-07-24.) **The commerce critical path is complete.** Everything remaining (#42/#43, #33, #13, #27, #29, arch lane) is parallel-safe, gated only by D7b/D12 and the ops inputs named in their Notes.

---

## Architecture lane (review of 2026-07-04)

Six refactors from `/improve-codebase-architecture` (report vocabulary: seam/depth/locality). #34 shipped 2026-07-04 (`aa5b70f`) and #38 shipped 2026-07-13 (`0764def`); #35/#37 flipped `ready-for-agent` 2026-07-24 and head the Now queue; #36/#39 stay `architecture` + `needs-triage`. Summarized here:
...
| arch | [#35](https://github.com/novaspatial/nova/issues/35) | ready-for-agent, architecture | One storage seam (buckets/paths/signed URLs) |
| arch | [#36](https://github.com/novaspatial/nova/issues/36) | needs-triage, architecture | Extract `useCommentClock` |
| arch | [#37](https://github.com/novaspatial/nova/issues/37) | ready-for-agent, architecture | `requireProjectChild` authz helper |
...
Closed: #2–#12, #14–#26, #30, #31, #32, #34, #38, #40, #41 (see Completed; #28 is a merged PR, not an issue).
