...
| --- | --- | --- | --- |
| 1 | [#36](https://github.com/novaspatial/nova/issues/36) **arch** | **Extract `useCommentClock`** — first meaningful test coverage for the Listen step. | Triage go 2026-07-26; `ready-for-agent`. Strength: Strong. |
| 2 | [#42](https://github.com/novaspatial/nova/issues/42) + [#43](https://github.com/novaspatial/nova/issues/43) **bug** | **Insert-fence residuals** from the #40/#41 work. #42: delete-then-reattach of a freed Stripe intent id (needs a DB-level floor — intent tombstone or service-mediated Stripe insert; the #19 `add_ons` metadata check narrows the forgeable surface further but the same-shape resurrection stands). #43: direct PostgREST `pending_payment` inserts skip the route's rate limit + consent gate, and (since #26) can stamp an `applied_coupon_code` without a hold — if such a row is ever paid, consume burns that code's capacity (griefing-only; no price advantage). | Narrow, non-payment-integrity. Triage go 2026-07-26; take both as one slice. |
| 3 | [#33](https://github.com/novaspatial/nova/issues/33) **S23** | **Activate IndexNow in prod**: set `INDEXNOW_KEY`, verify live key + accepted ping, and **reconcile the apex→www redirect with D10**. Now also carries setting `CRON_SECRET` (the #27 purge cron is inert without it). | `ready-for-human` — **blocked on the client**: prod's Vercel + GoDaddy sit under the client's email (2026-07-26; recovery email drafted, key generated). |
...
(#23, the former no-gate top of this path, shipped 2026-07-04; #40/#41 shipped 2026-07-08; #9/#31/#38/#25 shipped 2026-07-13; #26 shipped 2026-07-15; #30, #19, #24, and the #35/#37 arch slice shipped 2026-07-24; #13 closed 2026-07-25; #27 shipped 2026-07-26.) **The commerce and lifecycle critical paths are complete.** Everything remaining (#42/#43, #33, #29, #36, #39) is parallel-safe, gated only by D12 and the ops inputs named in their Notes.

---

## Architecture lane (review of 2026-07-04)

Six refactors from `/improve-codebase-architecture` (report vocabulary: seam/depth/locality). #34 shipped 2026-07-04 (`aa5b70f`), #38 shipped 2026-07-13 (`0764def`), and the #35/#37 slice shipped 2026-07-24 (`c2e1a23`); #36 got its triage go 2026-07-26 (`ready-for-agent`); #39 stays `architecture` + `needs-triage`. Summarized here:
...
| [#36](https://github.com/novaspatial/nova/issues/36) | Strong | `useCommentClock` extraction; Listen step becomes testable | Ready (triage go 2026-07-26) |
...
| arch | [#36](https://github.com/novaspatial/nova/issues/36) | ready-for-agent, architecture | Extract `useCommentClock` |
| arch | [#39](https://github.com/novaspatial/nova/issues/39) | needs-triage, architecture | Blog facade (hydrated post) |
| bug | [#42](https://github.com/novaspatial/nova/issues/42) | ready-for-agent, bug | Delete-then-reattach of a freed Stripe intent id (DB floor) |
| bug | [#43](https://github.com/novaspatial/nova/issues/43) | ready-for-agent, bug | Direct PostgREST inserts bypass the checkout rate limit + consent gate |

Closed: #2–#27, #30, #31, #32, #34, #35, #37, #38, #40, #41 (see Completed; #28 is a merged PR, not an issue).
