...
**Done =** the 11 open work issues below closed (or consciously moved to *Deliberately not built*) and the open decisions in #1 answered.
...
| --- | --- | --- | --- |
| 1 | [#24](https://github.com/novaspatial/nova/issues/24) **S8** | **Order-confirmation email** on paid order. D13's sender answered 2026-07-14: launch from the existing `noreply@nova-spatial.com` Resend sender (subdomain swap later is config-only). Tax + coupon + add-on lines ready (`tax_cents`, `applied_coupon_code`, `add_ons` persisted; labels from `ADD_ON_LABELS`). | Reads `NOTIFIABLE_STATUSES` from the #34 seam; siteConfig origin for portal links. #26 landed 2026-07-15, so the sequencing gate is satisfied: the handler documents the post-claim order **consumption (must-500) → [#24 email, best-effort] → ack**. `ready-for-agent`. |
...
**Commerce chain: empty.** Every gate is answered (the 2026-07-13 decisions; the 2026-07-14 rulings on the sender, rush availability, and revisions) — #26 shipped 2026-07-15, #30 and #19 shipped 2026-07-24; #24, the last commerce slice, heads the Now queue above.
...
```text
#24                                                   ← head; receipt (sender = existing noreply; #26's webhook documents its post-claim slot)
```

(#23, the former no-gate top of this path, shipped 2026-07-04; #40/#41 shipped 2026-07-08; #9/#31/#38/#25 shipped 2026-07-13; #26 shipped 2026-07-15; #30 and #19 shipped 2026-07-24.) Everything else (#42/#43, #33, #13, #27, #29, arch lane) is parallel-safe. **No decision touches the path anymore** — #24 ships the receipt from the existing sender and closes the commerce lane.
...
Closed: #2–#12, #14–#23, #25, #26, #30, #31, #32, #34, #38, #40, #41 (see Completed; #28 is a merged PR, not an issue).
