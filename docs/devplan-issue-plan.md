...
**Done =** the 9 open work issues below closed (or consciously moved to *Deliberately not built*) and the open decisions in #1 answered.
...
| --- | --- | --- | --- |
| 1 | [#33](https://github.com/novaspatial/nova/issues/33) **S23** | **Activate IndexNow in prod**: set `INDEXNOW_KEY`, verify live key + accepted ping, and **reconcile the apex→www redirect with D10**. | Pure ops + verification; no code gaps. `ready-for-human` — needs prod env access (`INDEXNOW_KEY`, DNS/redirect). |
| 2 | [#36](https://github.com/novaspatial/nova/issues/36) **arch** | **Extract `useCommentClock`** — first meaningful test coverage for the Listen step. | Independent; fill capacity between gated slices. Strength: Strong. |
| 3 | [#42](https://github.com/novaspatial/nova/issues/42) + [#43](https://github.com/novaspatial/nova/issues/43) **bug** | **Insert-fence residuals** from the #40/#41 work. #42: delete-then-reattach of a freed Stripe intent id (needs a DB-level floor — intent tombstone or service-mediated Stripe insert; the #19 `add_ons` metadata check narrows the forgeable surface further but the same-shape resurrection stands). #43: direct PostgREST `pending_payment` inserts skip the route's rate limit + consent gate, and (since #26) can stamp an `applied_coupon_code` without a hold — if such a row is ever paid, consume burns that code's capacity (griefing-only; no price advantage). | Narrow, non-payment-integrity; `needs-triage`. Take with the arch lane. |
...
**Lifecycle queue:** D7b ─> [#27](https://github.com/novaspatial/nova/issues/27) **S18** delivery anchor + 90-day purge (D7 answered 2026-07-14: Vercel Cron; scope in practice = stem + mix rows since the 2026-07-25 deliverables removal — confirm at pickup; only the record question — hard-delete vs tombstone — remains. Reuses `projectCleanup` + the shipped #35 storage seam (`bucketFor`/`removeStorageObjects`); stamps `delivered_at` at the PATCH transition point the shipped #34 seam made explicit). (#13 S17 closed 2026-07-25 as already-live — see Completed.)
...
(#23, the former no-gate top of this path, shipped 2026-07-04; #40/#41 shipped 2026-07-08; #9/#31/#38/#25 shipped 2026-07-13; #26 shipped 2026-07-15; #30, #19, #24, and the #35/#37 arch slice shipped 2026-07-24; #13 closed 2026-07-25.) **The commerce critical path is complete.** Everything remaining (#42/#43, #33, #27, #29, #36, #39) is parallel-safe, gated only by D7b/D12 and the ops inputs named in their Notes.

---

## Architecture lane (review of 2026-07-04)

Six refactors from `/improve-codebase-architecture` (report vocabulary: seam/depth/locality). #34 shipped 2026-07-04 (`aa5b70f`), #38 shipped 2026-07-13 (`0764def`), and the #35/#37 slice shipped 2026-07-24 (`c2e1a23`); #36/#39 stay `architecture` + `needs-triage`. Summarized here:
...
Closed: #2–#26, #30, #31, #32, #34, #35, #37, #38, #40, #41 (see Completed; #28 is a merged PR, not an issue).
