...
IndexNow went **live in production 2026-07-27** ([#33](https://github.com/novaspatial/nova/issues/33) — Vercel access recovered; `INDEXNOW_KEY` set, key URL 200s on the apex, redirect flipped to www→apex per D10, and `CRON_SECRET` armed the #27 purge cron in the same pass. #33's one open item: verify an accepted ping in the logs on the next publish).
...
| --- | --- | --- | --- |
| 1 | [#33](https://github.com/novaspatial/nova/issues/33) **S23** | **Activate IndexNow in prod** — **ops done 2026-07-27** (Vercel access recovered): `INDEXNOW_KEY` set (key URL 200s on the apex), redirect flipped www→apex per D10, `CRON_SECRET` set (#27 cron armed, endpoint 401s a bad bearer), and a stray Production-scoped `PAYMENTS_DEV_BYPASS` deleted. | One item left before closing: see an accepted ping in the Vercel logs on the next blog publish (`[indexnow] ping accepted` — success log added 2026-07-27). |
