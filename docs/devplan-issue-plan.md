...
**Done =** the remaining open work (now just #29, with #39 bundled in) closed (or consciously moved to *Deliberately not built*) and the open decisions in #1 answered.

## How an agent picks work

1. Work the **Now queue** top-down. The queue is currently **empty** (#33 closed 2026-07-27) — the next work fires from the gated chains below.
...
IndexNow is **live and verified in production** ([#33](https://github.com/novaspatial/nova/issues/33) closed 2026-07-27 — Vercel access recovered; `INDEXNOW_KEY` set, key URL 200s on the apex, redirect flipped to www→apex per D10, and `CRON_SECRET` armed the #27 purge cron in the same pass; the same day a `/blog/admin` re-save produced `[indexnow] ping accepted` 202/200 in the prod runtime logs, with the engine's key-verification fetch of `/indexnow-key.txt` returning 200).
...
| --- | --- | --- | --- |
| — | *(empty)* | #33 closed 2026-07-27 (ping verified — see Completed). Next work fires from the gated chains: #29 (+#39 bundled) on the studio `sameAs` URLs; Part B on D12. | |
...
**Lifecycle queue: done.** D7b answered 2026-07-26 (tombstone) and [#27](https://github.com/novaspatial/nova/issues/27) shipped the same day — see Completed. The production `CRON_SECRET` landed 2026-07-27 in #33's ops pass — the purge cron is armed. (#13 S17 closed 2026-07-25 as already-live — see Completed.)
...
| S23 | #33 | IndexNow production activation (ops, no code change): `INDEXNOW_KEY` + `CRON_SECRET` set 2026-07-27 (Vercel access recovered), key URL 200 on the apex, primary domain flipped www→apex 308 per D10, stray Production `PAYMENTS_DEV_BYPASS` deleted; verified same day — a `/blog/admin` re-save fired `[indexnow] ping accepted` 202/200 in the prod runtime logs and the engine's key-verification `GET /indexnow-key.txt` returned 200 (success log `781edba`; ARCHITECTURE.md caveat off in `151ecf4`) |
...
Closed: #2–#27, #30–#38, #40–#43 (see Completed; #28 is a merged PR, not an issue).
