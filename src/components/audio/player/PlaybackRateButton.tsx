...
export function PlaybackRateButton({ player }: { player: AudioPlayerAPI }) {
  const [playbackRate, setPlaybackRate] = useState(playbackRates[0])

  return (
    <button
      type="button"
      className="relative flex size-6 items-center justify-center rounded-md text-zinc-400 hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
      onClick={() => {
